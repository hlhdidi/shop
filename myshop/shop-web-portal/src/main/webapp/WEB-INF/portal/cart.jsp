...
//结算
function trueBuy(){
	$("#isLoginInForm").val($("#isLogin").val());
	$("#usernameInForm").val($("#username").val());
	$("#jvForm").submit();
}
</script>
<body>
...
<c:if test="${fn:length(buyerCart.items) != 0}">
	<div class="w cart">
		...
		<form id="jvForm" action="${pageContext.request.contextPath }/portal/order/toOrder" method="post">
		<div id="show">
			...
			<div class="cart-inner">
				<div class="cart-thead clearfix">
					...
					<input type="hidden" name="isLogin" id="isLoginInForm">
					<input type="hidden" id="usernameInForm" name="username" value="">	
				</div>
						...
						<div id="product_11345721" class="item item_selected ">
							<div class="item_form clearfix">
								...
								<div class="cell p-inventory stock-11345721">
									<c:if test="${item.isHave==false }">无货</c:if>
									<c:if test="${item.isHave==true }">有货</c:if>
								</div>
