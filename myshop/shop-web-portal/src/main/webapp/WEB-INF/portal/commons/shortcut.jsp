...
<script type="text/javascript">
	$(function() {
		//发送ajax请求判断是否已经登录
		$.ajax({
			url:"http://127.0.0.1:8580/isLogin.aspx",
			type:"post",
			dataType:"jsonp",	//可以写入数据
			success:function(data) {
				...
				if(data.result==0) {
					...
					$("#login").hide();
					$("#regist").hide();
					$("#isLogin").val("true");//在加入购物车的时候方便携带.
	...
	function login() {
		location.href="http://127.0.0.1:8580/login.aspx?resource="+encodeURIComponent(location.href);
	}
	function regist() {
		location.href="http://127.0.0.1:8580/toRegist.aspx";
	}
</script>
