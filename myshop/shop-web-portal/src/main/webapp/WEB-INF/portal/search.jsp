...
<!-- header end -->
<form action="${pageContext.request.contextPath }/portal/product/list" method="post" id="searchForm">
	<input type="hidden" id="priceInput" name="price" value="${price }"/>
	<input type="hidden" id="brandIdInput" name="brandId" value="${brandId }"/>
			...
			<div class="sl-b-selected J_brandSelected">
				<span class="crumbs-arrow">已选条件：</span>
					<c:forEach items="${map }" var="m">
						<a title="${m.value }"  href="javascript:hideKeyword('${m.key }');" class="crumb-select-item">
...
<div id="J_goodsList" class="goods-list-v1 gl-type-1 J-goods-list">
	<ul class="gl-warp clearfix" data-tpl="1">
		...
		<li data-sku="1711416562" class="gl-item">
			<div class="gl-i-wrap">
				<div class="p-img">
					<a href="javascript:;" onclick="window.open('/portal/product/detail/${product.id}')" style="position: relative;">
						<img width="220" height="220" class="err-product"  src="${product.firstImg}">
				...
				<div class="p-name p-name-type-2">
					<a target="" title="满129立减10,199减20优惠券,支持货到付款" href="javascript:;" onclick="window.open('http://127.0.0.1:8180/html/product/${product.id}.html')">
						<em>${product.name }</em>
					</a>
				</div>
				<div class="p-commit">
